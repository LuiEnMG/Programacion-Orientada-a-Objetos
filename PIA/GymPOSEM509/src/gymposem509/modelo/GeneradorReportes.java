...
public class GeneradorReportes implements Runnable {
    ...
    private final String nombreUsuario; // para registrar quién generoó el reporte

    public GeneradorReportes(TipoReporte tipo) {
        this(tipo, "sistema");
    }
    
    public GeneradorReportes(TipoReporte tipo, String nombreUsuario) {
        this.tipo = tipo;
    ...
    private void generarReporte() throws IOException {
        ...
        try (FileWriter fw = new FileWriter(archivo, true);
            ...
            out.println("===== GYM POSEM 509 - REPORTE " + tipo.name() + " =====");
            out.println("Generado por: " + nombreUsuario);
            out.println("Fecha de generación: " + new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(new Date()));
            out.println();
    ...
    private void generarReporteClientes(PrintWriter out) {
        GestionClientesMorales gestor = new GestionClientesMorales();
    ...
    private void generarReporteEmpleados(PrintWriter out) {
        GestionEmpleadosMorales gestor = new GestionEmpleadosMorales();
    ...
    private void generarReporteInventario(PrintWriter out) {
        GestionInventarioMorales gestor = new GestionInventarioMorales();
    ...
    private void generarReporteMembresias(PrintWriter out) {
        SistemaMembresia2608 sistema = new SistemaMembresia2608();
        List<Membresia> membresias = new ArrayList<>();
        List<Membresia> cargadas = sistema.cargarMembresias(membresias);
        
        if (cargadas == null || cargadas.isEmpty()) {
            out.println("No hay membresías registradas.");
            return;
        }
        
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        
        out.println("Total de membresías: " + cargadas.size());
        out.println("------------------------------------");
        for (Membresia m : cargadas) {
            out.println("ID Cliente: " + m.getId_cliente());
